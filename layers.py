...
def linear_fa_backward_hook(module, grad_input, grad_output):
    if grad_input[1] is not None:
        grad_input_fa = grad_output[0].mm(module.fb_weight)

        return (grad_input[0], grad_input_fa, grad_input[2])

def conv2d_fa_backward_hook(module, grad_input, grad_output):
    if grad_input[0] is not None:
        grad_input_fa = torch.nn.grad.conv2d_input(grad_input[0].size(), module.fb_weight, grad_output[0], stride=module.stride, padding=module.padding, dilation=module.dilation, groups=module.groups)
...
class LinearFA(nn.Module):
    ...
    def __init__(self, in_features, out_features, bias=True):
        ...
        self.fb_weight = Parameter(torch.Tensor(out_features, in_features), requires_grad=False)
        if bias:
    ...
    def decay_fb_weights(self, weight_decay):
        self.fb_weight.data -= weight_decay*self.fb_weight.data
...
class _ConvNdFA(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size, stride,
        ...
        if transposed:
            ...
            self.fb_weight = Parameter(torch.Tensor(
